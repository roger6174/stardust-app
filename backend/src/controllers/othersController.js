...
const addOtherDocument = async (req, res) => {
    const { title, description, notes, file_key, is_encrypted = 1 } = req.body;
    ...
    try {
        ...
        const [result] = await db.execute(
            ...
            [userId, 'OTHER', title, metadataValue, is_encrypted]
        );
...
const updateOtherDocument = async (req, res) => {
    ...
    const { title, description, notes, file_key, is_encrypted } = req.body;
    const userId = req.user.id;
    ...
    try {
        ...
        const [result] = await db.execute(
            ...
            [title, metadataValue, is_encrypted, assetId, userId, 'OTHER']
        );
...
const deleteOtherDocument = async (req, res) => {
    ...
    try {
        ...
        console.error('❌ [DELETE OTHER ERROR]:', error);
